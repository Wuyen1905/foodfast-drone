...
import { ORDER_STATUS_OPTIONS, OrderStatus as RestaurantOrderStatus, normalizeStatus } from '@/constants/orderStatus';
    ...
    const handleNewOrder = (event: CustomEvent) => {
      ...
      if (notification.restaurantId) {
        ...
        if (normalizedNotificationId === normalizedRestaurantId) {
          // Trigger a refresh by reloading orders from backend
          // This will be handled by the useOrders hook or component refresh
        }
      }
    };
                    ...
                    <>
                      ...
                      >
                        {(() => {
                          ...
                          const currentOption = ORDER_STATUS_OPTIONS.find(opt => opt.value === current);
                          const otherOptions = ORDER_STATUS_OPTIONS.filter(opt => opt.value !== current);
                          
                          return (
                            <>
                              {currentOption && (
                                <option key={currentOption.value} value={currentOption.value}>
                                  {currentOption.label} (Hiện tại)
                                </option>
                              )}
