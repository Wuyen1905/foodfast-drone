...
const Orders: React.FC = () => {
  ...
  const fetchOrdersByPhone = async (phoneNumber: string) => {
    const response = await fetch(`${API_BASE_URL}/orders?phone=${encodeURIComponent(phoneNumber)}`);
    if (!response.ok) {
  ...
  const handleStatusUpdate = (orderId: string, newStatus: string) => {
    ...
    if (phone.trim()) {
      fetchOrdersByPhone(phone.trim()).then(setFilteredOrders).catch(() => {});
    }
  };
  ...
  useEffect(() => {
    const unsubscribe = realtimeSocket.onOrderUpdate((updatedOrder) => {
      ...
      // Refresh list for customer (search by phone)
      if (!isAdmin() && phone.trim()) {
        fetchOrdersByPhone(phone.trim())
          .then(setFilteredOrders)
          .catch(() => {});
      }
    });
