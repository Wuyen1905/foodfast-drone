...
class SafeDevServer {
  ...
  async killPort(port) {
    this.log(`Checking if port ${port} is in use...`, 'info');
    
    try {
      // Use npx kill-port which works cross-platform (Windows, macOS, Linux)
      await this.runCommand('npx', ['kill-port', port.toString()]);
      this.log(`Port ${port} freed successfully`, 'success');
    } catch (error) {
      // Port might not be in use, which is fine
      this.log(`Port ${port} is available (or kill-port failed: ${error.message})`, 'info');
    }
  }
  ...
  async runCommand(command, args = [], options = {}) {
    return new Promise((resolve, reject) => {
      ...
      process.on('close', (code) => {
        // For kill-port, non-zero exit code might mean port wasn't in use, which is fine
        if (command === 'npx' && args[0] === 'kill-port') {
          resolve(); // Always resolve for kill-port
        } else if (code === 0) {
          resolve();
        } else {
      ...
      process.on('error', (error) => {
        // For kill-port, errors are acceptable (port might not be in use)
        if (command === 'npx' && args[0] === 'kill-port') {
  ...
  async start() {
    try {
      ...
      // Step 3: Kill any process using port 5173
      await this.killPort(5173);
      
      // Step 4: Start Vite with timeout protection
      this.startVite();
