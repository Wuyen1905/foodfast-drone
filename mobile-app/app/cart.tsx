...
import { View, Text, Image, Pressable, StyleSheet, ScrollView, AppState } from 'react-native';
import { router } from 'expo-router';
import { api } from '@/api';
import { theme } from '@/theme';
import { realtimeSocket } from '@/realtime/socket';
import { saveCartToStorage, loadCartFromStorage } from '@/hooks/useAppState';
...
export default function Cart() {
  ...
  const loadCart = useCallback(async () => {
    try {
      ...
      // If API fails, try to restore from storage (resilience to network issues)
  ...
  // Restore cart from storage on mount (resilience to app background/foreground)
  useEffect(() => {
  ...
  // Save cart when app goes to background (resilience to phone calls)
  useEffect(() => {
  ...
  return (
    <View style={styles.container}>
      ...
      <Pressable style={styles.cta} onPress={() => router.push('/checkout')}><Text style={{ color: '#fff' }}>Proceed to Checkout</Text></Pressable>
    </View>
  );
}
