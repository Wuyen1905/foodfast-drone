/**
 * Runtime Test: GET /api/restaurants
 * Tests live API call to fetch restaurants
 */

import axios from 'axios';
import { getBackendUrl } from '../../frontend-mobile/src/api/getBackendUrl';

async function testLiveRestaurants() {
  console.log('\n========================================');
  console.log('[RUNTIME TEST] GET /api/restaurants');
  console.log('========================================\n');
  ...
  try {
    ...
    // Construct full URL
    const fullURL = `${BASE_URL}/restaurants`;
    ...
    if (response.status === 200) {
      if (Array.isArray(response.data)) {
        ...
        console.log(`\n[RESULT] PASS`);
        console.log(`[SUMMARY] Backend returned ${itemCount} restaurants`);
    ...
    if (error.code === 'ECONNREFUSED') {
      ...
      rootCause = 'Endpoint not found (404) - restaurants endpoint may not exist';
...
export { testLiveRestaurants };

// If run directly, execute test
if (require.main === module) {
  testLiveRestaurants().then(() => {
    console.log('\n[TEST COMPLETE]');
    process.exit(0);
  }).catch((err) => {
    console.error('\n[TEST ERROR]', err);
