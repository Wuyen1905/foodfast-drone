 ...
 * ⚠️ TEST-ONLY FUNCTION - Do NOT use in production
 * This function simulates a successful VNPay payment for testing purposes only
 * In production, payments should go through the real VNPay gateway
 */
export const simulateVNPayPayment = (): Promise<{
  ...
  if (process.env.NODE_ENV === 'production') {
    console.error('[VNPay] simulateVNPayPayment should NOT be used in production!');
    return Promise.reject(new Error('Simulation function not allowed in production'));
  }
  
  // Test-only simulation
  return Promise.resolve({
    success: true,
    transactionId: `VNPAY_TEST_${Date.now()}`,
    message: 'Thanh toán thành công qua VNPay (TEST MODE)'
  });
};
...
export async function createVNPayUrl(amount: number, orderId: string) {
  const res = await api.post("/payment/vnpay/create", { amount, orderId });
  const data = res.data;
  if (!data || data.error) throw new Error(data?.error || "Payment error");
  return data.url;
}
