...
export const fetchAssistantContext = async (): Promise<AssistantContext | null> => {
  try {
    const response = await api.get(`/assistantContext`);
...
export const getBatteryTrend = async (): Promise<BatteryTrendPoint[]> => {
  try {
    ...
    try {
      const response = await api.get(`/batteryTrend`);
    ...
    const dronesResponse = await api.get(`/drones`).catch(() => ({ data: [] }));
