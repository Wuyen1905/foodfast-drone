...
export const fetchRealtimeDroneData = async (): Promise<DroneRealtimeData[]> => {
  try {
    ...
    const enhancedDrones: DroneRealtimeData[] = drones.map((drone: any) => {
      ...
      // Use speed from backend API only - no mock/simulated values
      let speed = drone.speed || 0;
      if (drone.speedMps && !speed) {
        // Convert m/s to km/h (multiply by 3.6) if backend provides speedMps
        speed = drone.speedMps * 3.6;
      }
      // If backend doesn't provide speed, keep it at 0 - don't simulate
      ...
      // Use ETA from backend API only - no mock calculations
      // If backend doesn't provide ETA, it will remain undefined
      const eta = drone.eta || undefined;
