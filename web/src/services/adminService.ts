...
export const getAllDrones = async (): Promise<AdminDrone[]> => {
  try {
    const response = await api.get(`/admin/drones`);
...
export const updateRestaurantStatus = async (id: string, status: 'active' | 'inactive'): Promise<boolean> => {
  try {
    await api.patch(`/admin/restaurants/${id}/status`, { isActive: status === 'active' });
    return true;
  } catch (error) {
...
export const suspendCustomer = async (customerId: string): Promise<boolean> => {
  try {
    await api.patch(`/admin/users/${customerId}/suspend`);
    return true;
  } catch (error) {
...
export const reactivateCustomer = async (customerId: string): Promise<boolean> => {
  try {
    await api.patch(`/admin/users/${customerId}/reactivate`);
    return true;
  } catch (error) {
...
// Get system logs
// Note: Backend /api/admin/system-logs endpoint does not exist yet
export const getSystemLogs = async (): Promise<any[]> => {
  try {
    // TODO: Implement backend /api/admin/system-logs endpoint
    console.warn('[adminService] Backend /api/admin/system-logs endpoint does not exist yet.');
    return []; // Return empty array - don't fake data
  } catch (error) {
...
// Note: Backend /api/admin/emergency-override endpoint does not exist yet
export const performEmergencyOverride = async (
  ...
  try {
    // TODO: Implement backend /api/admin/emergency-override endpoint
    console.warn('[adminService] Backend /api/admin/emergency-override endpoint does not exist yet. Action not performed:', { targetType, targetId, action });
